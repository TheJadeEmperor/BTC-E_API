...
class BTCeAPI {
    ...
    public function getSellPrice($pair) {
        $json = $this->retrieveJSON($this->public_api.'ticker/'.$pair);        
        return $json[$pair]['sell'];
    }

    public function getBuyPrice($pair) {
        $json = $this->retrieveJSON($this->public_api.'ticker/'.$pair);        
        return $json[$pair]['buy'];
    }

    public function getHighPrice($pair) {
        $json = $this->retrieveJSON($this->public_api.'ticker/'.$pair);        
        return $json[$pair]['high'];
    }
    
    public function getLowPrice($pair) {
        $json = $this->retrieveJSON($this->public_api.'ticker/'.$pair);        
        return $json[$pair]['low'];
    }
